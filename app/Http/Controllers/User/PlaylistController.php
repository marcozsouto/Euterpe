...
{
    function form_user_playlist(){
        $this->authorize("isUserLogged", User::class);
        $playlists = Playlist::where('user_id',Auth::user()->id)->get();
        return view('user.playlist',['playlists' => $playlists]);
    }

    function form_playlist_edit($id){
        $this->authorize("isUserLogged", User::class);
        $playlist = Playlist::find($id);
        return view('user.editPlaylist',['playlist' => $playlist]);
    ...
    public function edit(Request $request){
        try{
            ...
            return Redirect::back();
                ...
                return redirect("home/playlist/edit/".$request->id)->withErrors($exception->getValidator())->withInput();
            }
                 
    }

    function form_user_playlist_new(){
        $this->authorize("isUserLogged", User::class);
        return view('user.createPlaylist');
    ...
    public function create(Request $request){
        try{    
            ...
            return redirect('home/playlist/new')->withErrors($exception->getValidator())->withInput();
        }
    ...
    function action(Request $request){
        ...
        if($query != ''){
            $data = Playlist::where([['name', 'like', '%'.$query.'%'],['user_id',Auth::user()->id]])->get();

        }else{

            $data = Playlist::where('user_id',Auth::user()->id)->get();
        }
    ...
    public function show_playlist($id){
        $this->authorize("isUserLogged", User::class);
        $playlist = Playlist::find($id);
        return view('user.showPlaylist',['playlist' => $playlist]);
